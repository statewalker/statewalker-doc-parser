// Regexps below are shamelessly copied from "uc.micro" (MIT license).
//Control symbols
...
export const CHAR_SPACE = 1;
export const CHAR_EOL = 2;
export const CHAR_DIGIT = 4;
export const CHAR_PUNCTUATION = 16;
export const CHAR_FORMAT = 32;
export const CHAR_CONTROL = 64;
export const CHAR_ANY = 128;
export const EOS = 512;
...
export function detectCharType(char: string): number {
  ...
  if (
    ...
    // Corresponds to the Unicode class Zs
    return CHAR_SPACE;
  } else if (char >= "0" && char <= "9") {
    return CHAR_DIGIT;
  } else if (char.match(Cc)) {
    return CHAR_CONTROL;
  } else if (char.match(Cf)) {
    return CHAR_FORMAT;
  } else if (char === "`") {
    return CHAR_PUNCTUATION;
  } else if (char.match(P)) {
    return CHAR_PUNCTUATION;
  } else if (char.match(Any)) {
    return CHAR_ANY;
  }
  return EOS;
}
